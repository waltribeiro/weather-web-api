...
function searchWeather(val) {
  var iconSun =
    "<i class='fas fa-sun fa-7x d-flex justify-content-center' style='margin:10px'></i>";
  var iconRain =
    "<i class='fas fa-cloud-rain fa-7x d-flex justify-content-center' style='margin:10px'></i>";
  var iconSnowflake =
    "<i class='far fa-snowflake fa-7x d-flex justify-content-center' style='margin:10px'></i>";
  var iconCloud =
    "<i class='fas fa-cloud fa-7x d-flex justify-content-center' style='margin:10px'></i>";
  ...
  $.ajax({
    ...
    if (localStorageHistory.indexOf(val) === -1) {
      ...
      var li = $("<li>")
        .addClass(
          "list-group-item list-group-item-action list-group-item text-dark"
        )
        .text(val);
    ...
    var bigTemperature1 = (
      Math.floor(response.main.temp - 273.15) * 1.8 +
      32
    ).toFixed(0);
...
//MOMENT.js
var currentTime = $("#currentDayPlus0").text(
  moment()
    .format("ddd MMM D")
    .toUpperCase()
);
var currentTime1 = $("#currentDayPlus1").text(
  moment()
    .add(1, "days")
    .format("ddd / D")
    .toUpperCase()
);
var currentTime2 = $("#currentDayPlus2").text(
  moment()
    .add(2, "days")
    .format("ddd / D")
    .toUpperCase()
);
var currentTime3 = $("#currentDayPlus3").text(
  moment()
    .add(3, "days")
    .format("ddd / D")
    .toUpperCase()
);
var currentTime4 = $("#currentDayPlus4").text(
  moment()
    .add(4, "days")
    .format("ddd / D")
    .toUpperCase()
);
var currentTime5 = $("#currentDayPlus5").text(
  moment()
    .add(5, "days")
    .format("ddd / D")
    .toUpperCase()
...
function searchForecast(val) {
  ...
  $.ajax({
    ...
    for (x = 0; x < 6; x++) {
      ...
      $("#smallTemperature" + [x]).append(
        (Math.floor(weatherTemp - 273.15) * 1.8 + 32).toFixed(0) + "°"
      );
    }
    ...
    var littleTemperature1 =
      (Math.floor(response.main.temp - 273.15) * 1.8 + 32).toFixed(0) + "°";
